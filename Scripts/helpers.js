...
exports.isBridgetownProject = function() {
  let gemfilePath
  ...
  if (gemfile.indexOf("gem 'bridgetown'") !== -1 || gemfile.indexOf('gem "bridgetown"') !== -1) {
    return true
  } else {
    return false
  }
...
exports.bridgetownNotes = async function() {
  if (!nova.workspace.bridgetownDetected) return []

  return new Promise((resolve, reject) => {
    const process = new Process("/usr/bin/env", {
      cwd: nova.workspace.path,
      args: ["bin/bridgetown", "notes"],
    ...
    process.onDidExit((status) => {
      ...
      if (str.length == 0) { return reject("bridgetown notes is empty") }

const { bridgetownNotes } = require("../helpers")

class NotesView {
  constructor() {
    this.tree = new TreeView("tommasonegri.bridgetown.sidebar.notes", {
      dataProvider: this
    })
    this.rootItems = []
  ...
  async fetchNotes() {
    ...
    try {
      const notes = await bridgetownNotes()
      ...
      console.error("Something went wrong trying to fetch Bridgetown Notes:", err)
    }
  }

  registerCommands() {
    nova.commands.register("tommasonegri.bridgetown.commands.sidebar.notes.openFile", () => {
    ...
    nova.commands.register("tommasonegri.bridgetown.commands.sidebar.notes.reload", () => {
      this.fetchNotes()
    })
  }
}
